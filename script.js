...
// Mobile control buttons (touch/mouse)
const btnUp = document.getElementById('btnUp');
const btnLeft = document.getElementById('btnLeft');
const btnRight = document.getElementById('btnRight');
const btnDown = document.getElementById('btnDown');
const btnRestartMobile = document.getElementById('btnRestart');

function wireButton(btn, x, y){
  if(!btn) return;
  btn.addEventListener('touchstart', e=>{ e.preventDefault(); setDir(x,y); }, {passive:false});
  btn.addEventListener('mousedown', e=>{ e.preventDefault(); setDir(x,y); });
}

wireButton(btnUp, 0, -1);
wireButton(btnLeft, -1, 0);
wireButton(btnRight, 1, 0);
wireButton(btnDown, 0, 1);
if(btnRestartMobile){ btnRestartMobile.addEventListener('click', e=>{ resetGame(); running = true; hideGameOver(); hideStart(); }); }
